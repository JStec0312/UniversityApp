...
def test_create_user(db):
    ...
    user = User(email="test@example.com", hashed_password="test", display_name="Test User", university_id=0)
...
def test_get_by_username(db):
    user_repo = RepositoryFactory(db).get_user_repository()
    user_repo.create(User(email="test@gmail.com", hashed_password="test", display_name="terminator123", university_id=0))
